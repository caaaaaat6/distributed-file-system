...
class FileServer(fileService_pb2_grpc.FileserviceServicer):
    ...
    def DownloadFile(self, request, context):
        ...
        if (self.fileExists(request.username, request.filename) == False):
            return fileService_pb2.FileData(username=request.username, filename=request.filename,
                                            data=bytes("", 'utf-8'))
        ...
        if (channel1):
            ...
            responses = stub.DownloadFile(
                fileService_pb2.FileInfo(username=request.username, filename=request.filename))
